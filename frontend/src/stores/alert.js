...
export const useAlertStore = defineStore('alert', () => {
    ...
    const _hideAlert = (result = undefined, isCanceled = false) => {
    ...
    const handleButtonClick = (buttonValue) => {
        _hideAlert(buttonValue, false); // 正常な解決
    };

    const handleOverlayClick = () => {
        if (overlayClickToClose.value) {
            _hideAlert(undefined, true); // キャンセルとして拒否
        }
    };
    ...
    return {
        ...
        handleButtonClick,
        handleOverlayClick,
        // _hideAlert は内部的に使用するため、通常は外部に公開しない
    };
});